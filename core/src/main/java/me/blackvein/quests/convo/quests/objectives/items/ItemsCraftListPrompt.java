...
public class ItemsCraftListPrompt extends QuestsEditorNumericPrompt {
    ...
    public ItemsCraftListPrompt(final ItemsPrompt itemsPrompt, final ConversationContext context) {
        super(context);
    ...
    public String getAdditionalText(final ConversationContext context, final int number) {
        switch (number) {
            case 1:
                if (context.getSessionData(itemsPrompt.pref + CK.S_CRAFT_ITEMS) == null) {
                    return ChatColor.GRAY + "(" + Lang.get("noneSet") + ")";
                } else {
                    final StringBuilder text = new StringBuilder();
                    final List<ItemStack> craftItems
                            = (List<ItemStack>) context.getSessionData(itemsPrompt.pref + CK.S_CRAFT_ITEMS);
                    if (craftItems != null) {
    ...
    public @NotNull String getBasicPromptText(final ConversationContext context) {
        ...
        if (context.getSessionData("tempStack") != null) {
            if (context.getSessionData(itemsPrompt.pref + CK.S_CRAFT_ITEMS) != null) {
                final List<ItemStack> items = (List<ItemStack>) context.getSessionData(itemsPrompt.pref + CK.S_CRAFT_ITEMS);
                if (items != null) {
                    items.add((ItemStack) context.getSessionData("tempStack"));
                    context.setSessionData(itemsPrompt.pref + CK.S_CRAFT_ITEMS, items);
                }
            } else {
    ...
    protected Prompt acceptValidatedInput(final @NotNull ConversationContext context, final Number input) {
        switch (input.intValue()) {
            ...
            case 2:
                ...
                context.setSessionData(itemsPrompt.pref + CK.S_CRAFT_ITEMS, null);

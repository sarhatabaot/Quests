...
public class ItemsSmeltListPrompt extends QuestsEditorNumericPrompt {
    ...
    public ItemsSmeltListPrompt(final ItemsPrompt itemsPrompt, final ConversationContext context) {
        super(context);
    ...
    public String getAdditionalText(final ConversationContext context, final int number) {
        switch (number) {
            case 1:
                if (context.getSessionData(itemsPrompt.pref + CK.S_SMELT_ITEMS) == null) {
                    return ChatColor.GRAY + "(" + Lang.get("noneSet") + ")";
                } else {
                    final StringBuilder text = new StringBuilder();
                    final List<ItemStack> smeltItems
                            = (List<ItemStack>) context.getSessionData(itemsPrompt.pref + CK.S_SMELT_ITEMS);
                    if (smeltItems != null) {
    ...
    public @NotNull String getBasicPromptText(final ConversationContext context) {
        ...
        if (context.getSessionData("tempStack") != null) {
            if (context.getSessionData(itemsPrompt.pref + CK.S_SMELT_ITEMS) != null) {
                final List<ItemStack> items = (List<ItemStack>) context.getSessionData(itemsPrompt.pref + CK.S_SMELT_ITEMS);
                if (items != null) {
                    items.add((ItemStack) context.getSessionData("tempStack"));
                    context.setSessionData(itemsPrompt.pref + CK.S_SMELT_ITEMS, items);
                }
            } else {
    ...
    protected Prompt acceptValidatedInput(final @NotNull ConversationContext context, final Number input) {
        switch (input.intValue()) {
            ...
            case 2:
                ...
                context.setSessionData(itemsPrompt.pref + CK.S_SMELT_ITEMS, null);
